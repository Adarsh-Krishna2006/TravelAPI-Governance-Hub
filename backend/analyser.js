...
const SYNONYMS = {
  customer: ['guest', 'client', 'user', 'passenger', 'payer', 'account', 'customer'],
  id: ['identifier', 'code', 'key', 'ref', 'reference', 'num', 'number', 'id'],
  booking: ['reservation', 'order', 'stay', 'itinerary', 'booking'],
  date: ['time', 'timestamp', 'created', 'scheduled', 'date', 'calendar', 'checkin', 'checkout', 'arrival', 'departure'],
  amount: ['price', 'cost', 'total', 'value', 'payment', 'charge', 'amount', 'fee', 'val', 'sum'],
  currency: ['curr', 'symbol', 'unit', 'currency', 'iso', 'code'],
  status: ['state', 'stage', 'condition', 'phase', 'status', 'label'],
  transaction: ['tx', 'txn', 'payment', 'charge', 'transaction'],
  card: ['cc', 'number', 'hash', 'obfuscated', 'card'],
  flight: ['air', 'plane', 'route', 'trip', 'flight'],
  hotel: ['property', 'room', 'stay', 'lodging', 'hotel']
};
...
function getRouteSimilarity(routeA, routeB) {
  const normA = normalizeRoute(routeA);
  const normB = normalizeRoute(routeB);

  if (normA === normB) return 1.0;

  const segsA = normA.split('/').filter(s => s.length > 0).map(s => getSynonymGroup(s));
  const segsB = normB.split('/').filter(s => s.length > 0).map(s => getSynonymGroup(s));
...
export function analyzeBaselinePair(apiA, apiB) {
  const routeScore = getRouteSimilarity(apiA.gatewayBaseUrl || apiA.gatewayRoute, apiB.gatewayBaseUrl || apiB.gatewayRoute);
  const methodScore = apiA.method === apiB.method ? 1.0 : 0.0;
...
export function analyzeEnhancedPair(apiA, apiB, settings = {}) {
  ...
  const fieldsScore = inputMatches / totalInputUnique;

  // 5. Semantic similarity (concept overlap + description similarity)
  ...
  const semanticsScore = semanticCount > 0 ? (semanticMatchesSum / Math.max(1, fieldsA.length)) : 0;
  ...
  for (const oA of outA) {
    if (outB.some(oB => oB.name === oA.name || CONCEPT_MAP[oA.name] === CONCEPT_MAP[oB.name])) {
      outputMatches++;
    }
  }
  const outputScore = outputMatches / Math.max(1, outA.length + outB.length - outputMatches);
  ...
  if (methodScore === 1.0) evidenceCheckmarks.push(`✓ Same HTTP method (${apiA.method})`);
